...
__global double2* Ip_gl)
{
	...
	//printf("iX=%i, iY=%i, theta=%g, psi=%g\n", iX, iY, ddtheta, ddpsi);
	...
	double gCenter = -PI - 0.5*dstep;

	for (gInt = 0; gInt < jintervals; gInt++) {
		gCenter += dstep;
		for (j = 0; j < jend; j++) {
			//printf("j= %i, tg=%g, ag= %g\n", j, tg[j], ag[j]);
			double gNode = tg[j]*0.5*dstep + gCenter;
			//if ((iX==0) && (iY==0)) printf("%g\n", gNode);
